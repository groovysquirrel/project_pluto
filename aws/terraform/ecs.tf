  ...
  container_definitions = jsonencode([
    ...
    # =========================================================================
    {
      ...
      environment = [
        ...
        #
        # WORKAROUND for oauth2-proxy bug #3165: X-Forwarded-User contains Cognito sub (GUID)
        # instead of email, even with USER_ID_CLAIM="email" set. Bug confirmed still exists
        # in v7.14.2 (Jan 2026). Using X-Forwarded-Email for both fields as workaround.
        # Users can manually update their display names in OpenWebUI after account creation.
        ...
        # NOTE: CORS_ALLOW_ORIGIN is intentionally NOT set
        # Setting it breaks streaming responses from LiteLLM (SSE format)
        # OpenWebUI default CORS handling works correctly with oauth2-proxy
      ...
      secrets = [
        ...
        # NOTE: SCIM_TOKEN removed - not needed with oauth2-proxy trusted header authentication
      ]
      mountPoints = [
        {
