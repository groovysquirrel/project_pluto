...
read -r -p "Route53 hosted zone used at deploy (example.com): " HOSTED_ZONE_NAME
if [ -z "$HOSTED_ZONE_NAME" ]; then
    print_error "Hosted zone name is required."
fi

read -r -p "Optional subdomain prefix used at deploy (press enter for none): " SUBDOMAIN_PREFIX
